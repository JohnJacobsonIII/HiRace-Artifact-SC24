 ...
 **-------------------------------------------------------
 */
__global__ void Fan1(float *_m_cuda, float *_a_cuda, int Size, int t,
                     uint64_cu *__hr_m_cuda, uint64_cu *__hr_a_cuda)
{   
  /**** HIRACE STUFF ****/
  HiRaceDataWrap<float> m_cuda = _m_cuda;
  HiRaceDataWrap<float> a_cuda = _a_cuda;
   
  m_cuda.setMetadata(__hr_m_cuda);
  a_cuda.setMetadata(__hr_a_cuda);
   
  m_cuda.setScope(Scope::Global);
  a_cuda.setScope(Scope::Global);
...
__global__ void Fan2(float *_m_cuda, float *_a_cuda, float *_b_cuda,int Size, int j1, int t,
                     uint64_cu *__hr_m_cuda, uint64_cu *__hr_a_cuda, uint64_cu *__hr_b_cuda)
{
  /**** HIRACE STUFF ****/
  HiRaceDataWrap<float> m_cuda         = _m_cuda;
  HiRaceDataWrap<float> a_cuda = _a_cuda;
  HiRaceDataWrap<float> b_cuda  = _b_cuda;
   
  m_cuda.setMetadata(__hr_m_cuda);
  a_cuda.setMetadata(__hr_a_cuda);
  b_cuda.setMetadata(__hr_b_cuda);
   
  m_cuda.setScope(Scope::Global);
  a_cuda.setScope(Scope::Global);
  b_cuda.setScope(Scope::Global);
