...
__global__ void 
lud_diagonal(float *m, int matrix_dim, int offset,uint64_cu *__hr_d_m)
{
  int i,j;
...
void lud_cuda(float *m, int matrix_dim, uint64_cu *__hr_d_m)
{
  int i=0;
  ...
  for (i=0; i < matrix_dim-BLOCK_SIZE; i += BLOCK_SIZE) {
      lud_diagonal<<<1, BLOCK_SIZE>>>(m, matrix_dim, i, __hr_d_m);
      lud_perimeter<<<(matrix_dim-i)/BLOCK_SIZE-1, BLOCK_SIZE*2>>>(m, matrix_dim, i, __hr_d_m);
      dim3 dimGrid((matrix_dim-i)/BLOCK_SIZE-1, (matrix_dim-i)/BLOCK_SIZE-1);
      lud_internal<<<dimGrid, dimBlock>>>(m, matrix_dim, i, __hr_d_m); 
  }
  lud_diagonal<<<1,BLOCK_SIZE>>>(m, matrix_dim, i, __hr_d_m);
}
