...
) 
{
  ...
  __shared__ float __hr__temp[BLOCK_SIZE][BLOCK_SIZE];
  ...
  HiRaceDataWrap2D<float, BLOCK_SIZE, BLOCK_SIZE> temp = __hr__temp;
  __shared__ hr_shadowt (*__hr__temp_shadow)[BLOCK_SIZE];
  if (bid == 0) { // only check one block
    if (tid == 0) { // malloc with a single thread
      __hr__temp_shadow = new hr_shadowt[BLOCK_SIZE][BLOCK_SIZE];
      if(__hr__temp_shadow == NULL) { printf("HiRace: can't malloc shared metadata\n"); }
    }
    ...
    // while ( HEIGHT < offset );
    __syncthreads();
    temp = HiRaceDataWrap2D<float, BLOCK_SIZE, BLOCK_SIZE>(__hr__temp,
                                   __hr__temp_shadow,
                                   Scope::Block,
                                   &bcount,
                                   &wcount,
  ...
  #define temp temp.registerCallsite(__LINE__,__FILE__)
  /***** HIRACE END *****/

  // printf("im txyz, bxyz: %d %d %d, %d %d %d and gtid %d\n", threadIdx.x, threadIdx.y, threadIdx.z, blockIdx.x, blockIdx.y, blockIdx.z, tid);
...
#undef temp
  ...
  __syncthreads();
  if (tid == 0) { delete(__hr__temp_shadow); } 
  /***** HIRACE END *****/
}
