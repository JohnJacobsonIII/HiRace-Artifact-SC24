...
void ForwardSub();
void BackSub();
__global__ void Fan1(float *m, float *a, int Size, int t, uint64_cu *__hr_m, uint64_cu *__hr_a);
__global__ void Fan2(float *m, float *a, float *b,int Size, int j1, int t, uint64_cu *__hr_m, uint64_cu *__hr_a, uint64_cu *__hr_b);
 ...
 **-------------------------------------------------------
 */
__global__ void Fan1(float *__hr_m_cuda, float *__hr_a_cuda, int Size, int t,
                     uint64_cu *__hr_metadata_m_cuda, uint64_cu *__hr_metadata_a_cuda)
{   
  /**** HIRACE STUFF ****/
  unsigned __hr_bcount = 0, __hr_wcount = 0, __hr_swidx = 0;

  HIRACE_WRAP_DATA(float,m_cuda)
  HIRACE_WRAP_DATA(float,a_cuda)

  HIRACE_SET_DATA_GLOBAL(m_cuda)
  HIRACE_SET_DATA_GLOBAL(a_cuda)
   
  #define m_cuda m_cuda.registerCallsite(__LINE__,__FILE__)
  #define a_cuda a_cuda.registerCallsite(__LINE__,__FILE__)
  /**** HIRACE STUFF ****/
...
__global__ void Fan2(float *__hr_m_cuda, float *__hr_a_cuda, float *__hr_b_cuda,int Size, int j1, int t,
                     uint64_cu *__hr_metadata_m_cuda, uint64_cu *__hr_metadata_a_cuda, uint64_cu *__hr_metadata_b_cuda)
{
  /**** HIRACE STUFF ****/
  unsigned __hr_bcount = 0, __hr_wcount = 0, __hr_swidx = 0;
  ...
  HIRACE_WRAP_DATA(float,m_cuda)
  HIRACE_WRAP_DATA(float,a_cuda)
  HIRACE_WRAP_DATA(float,b_cuda)

  HIRACE_SET_DATA_GLOBAL(m_cuda)
  HIRACE_SET_DATA_GLOBAL(a_cuda)
  HIRACE_SET_DATA_GLOBAL(b_cuda)
  
  #define m_cuda m_cuda.registerCallsite(__LINE__,__FILE__)
  #define a_cuda a_cuda.registerCallsite(__LINE__,__FILE__)
  #define b_cuda b_cuda.registerCallsite(__LINE__,__FILE__)
  /**** HIRACE STUFF ****/
  ...
  if(threadIdx.x + blockIdx.x * blockDim.x >= Size-1-t) return;
	...
	auto __hr_tmp = a_cuda[Size*t+(yidx+t)];
	a_cuda[Size*(xidx+1+t)+(yidx+t)] -= m_cuda[Size*(xidx+1+t)+t] * __hr_tmp;
	//a_cuda[xidx+1+t][yidx+t] -= m_cuda[xidx+1+t][t] * a_cuda[t][yidx+t];
	if(yidx == 0){
		//printf("blockIdx.x:%d,threadIdx.x:%d,blockIdx.y:%d,threadIdx.y:%d,blockDim.x:%d,blockDim.y:%d\n",blockIdx.x,threadIdx.x,blockIdx.y,threadIdx.y,blockDim.x,blockDim.y);
		//printf("xidx:%d,yidx:%d\n",xidx,yidx);
    __hr_tmp = b_cuda[t];
		b_cuda[xidx+1+t] -= m_cuda[Size*(xidx+1+t)+(yidx+t)] * __hr_tmp;
	}
  /**** HIRACE STUFF ****/
  #undef m_cuda 
  #undef a_cuda 
...
void ForwardSub()
{
  ...
  /**** HIRACE STUFF ****/
  uint64_cu *__hr_m_cuda;
  uint64_cu *__hr_a_cuda;
  uint64_cu *__hr_b_cuda;
  
  cudaError_t code = cudaMalloc((void**) &__hr_m_cuda, Size * Size * sizeof(uint64_cu));
  if (code != cudaSuccess) { printf("hirace: %s on m_cuda\n", cudaGetErrorString(code)); }
  cudaMemset(__hr_m_cuda, 0, Size * Size * sizeof(uint64_cu));
  
  code = cudaMalloc((void**) &__hr_a_cuda, Size * Size * sizeof(uint64_cu));
  if (code != cudaSuccess) { printf("hirace: %s on a_cuda\n", cudaGetErrorString(code)); }
  cudaMemset(__hr_a_cuda, 0, Size * Size * sizeof(uint64_cu));
  
  code = cudaMalloc((void**) &__hr_b_cuda, Size * sizeof(uint64_cu));
  if (code != cudaSuccess) { printf("hirace: %s on b_cuda\n", cudaGetErrorString(code)); }
  cudaMemset(__hr_b_cuda, 0, Size * sizeof(uint64_cu));
  ...
  int block_size,grid_size;
    ...
    gettimeofday(&time_start, NULL);
	for (t=0; t<(Size-1); t++) {
		Fan1<<<dimGrid,dimBlock>>>(m_cuda,a_cuda,Size,t,__hr_m_cuda,__hr_a_cuda);
		cudaThreadSynchronize();
    
    cudaMemset(__hr_m_cuda, 0, Size * Size * sizeof(uint64_cu));
    cudaMemset(__hr_a_cuda, 0, Size * Size * sizeof(uint64_cu));
		
    Fan2<<<dimGridXY,dimBlockXY>>>(m_cuda,a_cuda,b_cuda,Size,Size-t,t,__hr_m_cuda,__hr_a_cuda,__hr_b_cuda);
		cudaThreadSynchronize();
	...
	cudaFree(a_cuda);
	cudaFree(b_cuda);
	cudaFree(__hr_m_cuda);
	cudaFree(__hr_a_cuda);
	cudaFree(__hr_b_cuda);
}
