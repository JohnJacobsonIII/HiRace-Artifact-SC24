...
runTest( int argc, char** argv) 
{
 ...
 for (iter=0; iter< niter; iter++){     
	...
	//Run kernels
	srad_cuda_1<<<dimGrid, dimBlock>>>(E_C, W_C, N_C, S_C, J_cuda, C_cuda, cols, rows, q0sqr); cudaDeviceSynchronize(); cudaError_t err = cudaGetLastError(); printf("CUDA error (kern1): %s\n", cudaGetErrorString(err));
	srad_cuda_2<<<dimGrid, dimBlock>>>(E_C, W_C, N_C, S_C, J_cuda, C_cuda, cols, rows, lambda, q0sqr); cudaDeviceSynchronize(); err = cudaGetLastError(); printf("CUDA error (kern2): %s\n", cudaGetErrorString(err));
