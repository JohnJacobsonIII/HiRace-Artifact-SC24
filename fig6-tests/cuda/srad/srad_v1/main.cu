...
int main(int argc, char *argv []){
  ...
  /**** HIRACE STUFF ****/
  int __hr_mem_size = sizeof(uint64_cu) * Ne;
	int __hr_mem_size_i = sizeof(uint64_cu) * Nr;											//
	int __hr_mem_size_j = sizeof(uint64_cu) * Nc;											//
  
  
  uint64_cu *__hr_d_I;
  cudaError_t code = cudaMalloc((void**) &__hr_d_I, __hr_mem_size);
  if (code != cudaSuccess) { printf("hirace: %s on d_I\n", cudaGetErrorString(code)); }
  cudaMemset(__hr_d_I, 0, __hr_mem_size);
  
  uint64_cu *__hr_d_iN;
  code = cudaMalloc((void**) &__hr_d_iN, __hr_mem_size_i);
  if (code != cudaSuccess) { printf("hirace: %s on d_iN\n", cudaGetErrorString(code)); }
  cudaMemset(__hr_d_iN, 0, __hr_mem_size_i);
  
  uint64_cu *__hr_d_iS;
  code = cudaMalloc((void**) &__hr_d_iS, __hr_mem_size_i);
  if (code != cudaSuccess) { printf("hirace: %s on d_iS\n", cudaGetErrorString(code)); }
  cudaMemset(__hr_d_iS, 0, __hr_mem_size_i);
  
  uint64_cu *__hr_d_jE;
  code = cudaMalloc((void**) &__hr_d_jE, __hr_mem_size_j);
  if (code != cudaSuccess) { printf("hirace: %s on d_jE\n", cudaGetErrorString(code)); }
  cudaMemset(__hr_d_jE, 0, __hr_mem_size_j);
  
  uint64_cu *__hr_d_jW;
  code = cudaMalloc((void**) &__hr_d_jW, __hr_mem_size_j);
  if (code != cudaSuccess) { printf("hirace: %s on d_jW\n", cudaGetErrorString(code)); }
  cudaMemset(__hr_d_jW, 0, __hr_mem_size_j);
  
  uint64_cu *__hr_d_sums;
  code = cudaMalloc((void**) &__hr_d_sums, __hr_mem_size);
  if (code != cudaSuccess) { printf("hirace: %s on d_sums\n", cudaGetErrorString(code)); }
  cudaMemset(__hr_d_sums, 0, __hr_mem_size);
  
  uint64_cu *__hr_d_sums2;
  code = cudaMalloc((void**) &__hr_d_sums2, __hr_mem_size);
  if (code != cudaSuccess) { printf("hirace: %s on d_sums2\n", cudaGetErrorString(code)); }
  cudaMemset(__hr_d_sums2, 0, __hr_mem_size);
  
  uint64_cu *__hr_d_dN;
  code = cudaMalloc((void**) &__hr_d_dN, __hr_mem_size);
  if (code != cudaSuccess) { printf("hirace: %s on d_dN\n", cudaGetErrorString(code)); }
  cudaMemset(__hr_d_dN, 0, __hr_mem_size);
  
  uint64_cu *__hr_d_dS;
  code = cudaMalloc((void**) &__hr_d_dS, __hr_mem_size);
  if (code != cudaSuccess) { printf("hirace: %s on d_dS\n", cudaGetErrorString(code)); }
  cudaMemset(__hr_d_dS, 0, __hr_mem_size);
  
  uint64_cu *__hr_d_dW;
  code = cudaMalloc((void**) &__hr_d_dW, __hr_mem_size);
  if (code != cudaSuccess) { printf("hirace: %s on d_dW\n", cudaGetErrorString(code)); }
  cudaMemset(__hr_d_dW, 0, __hr_mem_size);
  
  uint64_cu *__hr_d_dE;
  code = cudaMalloc((void**) &__hr_d_dE, __hr_mem_size);
  if (code != cudaSuccess) { printf("hirace: %s on d_dE\n", cudaGetErrorString(code)); }
  cudaMemset(__hr_d_dE, 0, __hr_mem_size);
  
  uint64_cu *__hr_d_c;
  code = cudaMalloc((void**) &__hr_d_c, __hr_mem_size);
  if (code != cudaSuccess) { printf("hirace: %s on d_c\n", cudaGetErrorString(code)); }
  cudaMemset(__hr_d_c, 0, __hr_mem_size);
 
  
  
  code = cudaGetLastError();
	if (code != cudaSuccess) {
		printf("HIRACE error: %s\n", cudaGetErrorString(code));
		exit(EXIT_FAILURE);
	}
  /**** HIRACE STUFF ****/
  ...
  cudaMemset(__hr_d_sums2, 0, __hr_mem_size);
		...
		srad<<<blocks, threads>>>(	lambda,									// SRAD coefficient 
									...
									d_I,									// output image
									__hr_d_iN,									// indices of North surrounding pixels
									__hr_d_iS,									// indices of South surrounding pixels
									__hr_d_jE,									// indices of East surrounding pixels
									__hr_d_jW,									// indices of West surrounding pixels
									__hr_d_dN,									// North derivative
									__hr_d_dS,									// South derivative
									__hr_d_dW,									// West derivative
									__hr_d_dE,									// East derivative
									__hr_d_c,									// diffusion coefficient
									__hr_d_I);									// output image
  cudaMemset(__hr_d_I, 0, __hr_mem_size);
  cudaMemset(__hr_d_iN, 0, __hr_mem_size_i);
  cudaMemset(__hr_d_iS, 0, __hr_mem_size_i);
  cudaMemset(__hr_d_jE, 0, __hr_mem_size_j);
  ...
  cudaFree(__hr_d_I);
	...
	cudaFree(__hr_d_dE);
	cudaFree(__hr_d_dW);
	cudaFree(__hr_d_sums);
	cudaFree(__hr_d_sums2);
